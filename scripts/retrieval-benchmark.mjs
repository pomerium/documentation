...
const PROMPTS_FILE = new URL('./retrieval-benchmark.prompts.json', import.meta.url);
...
function scoreRun(input, output) {
  ...
  const details = run.results.map((result) => {
    ...
    if (result.citedUrls != null && !Array.isArray(result.citedUrls)) {
      throw new Error(`Invalid benchmark file: citedUrls must be an array for result ${result.id}`);
    }
    if (seenIds.has(result.id)) {
      throw new Error(`Invalid benchmark file: duplicate result id ${result.id}`);
    }
  ...
  if (missingIds.length > 0) {
    throw new Error(`Invalid benchmark file: missing result ids ${missingIds.join(', ')}`);
  }
  ...
  const summary = {
    ...
    currentHostAccuracyBasis: 'all prompts in the run, including prompts with no citations',
  };

  summary.currentHostAccuracy = summary.totalPrompts === 0 ? 0 : summary.currentHostHits / summary.totalPrompts;
  summary.forbiddenHostRate = summary.totalPrompts === 0 ? 0 : summary.forbiddenHostHits / summary.totalPrompts;
