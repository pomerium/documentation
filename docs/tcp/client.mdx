...
See the "Configure Routes" section of [TCP Support](/tcp/readme#configure-routes) for more detailed information on TCP routes.
...
**Destination**: Matches the [From](/enterprise/reference/manage#from) value of the route, without the protocol. Always include the port specified in the route, and do not include the `https://` protocol.
...
**Client Certificate & Certificate Key File or Text**: For routes that require client certificates for [mTLS](/guides/mtls), you can provide the certificate and key file to the Pomerium Desktop client.
...
See [Release](overview/releases#pomerium-cli) to learn how to install pomerium-cli in your environment.

### Connect to a TCP Route

1. Invoke `pomerium-cli` with the `tcp` option, and provide the route to your service (As defined in [`from`](/reference/readme#from) in your Route specification).
...
For more examples and detailed usage information, see [TCP Support](/tcp/readme)

## Advanced Configuration

If Pomerium is listening on a port other than `443` (set with the [`address` key](/reference/readme#address)), the `pomerium-url` flag (CLI) or "Pomerium URL" field (GUI) is required. This specifies the address and port for the client to communicate over, while the standard URL defines the port assignment for the specific route. For example:
