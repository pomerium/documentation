...
See the "Configure Routes" section of [TCP Support](/tcp#configure-routes) for more detailed information on TCP routes.
...
1. Invoke `pomerium-cli` with the `tcp` option, and provide the route to your service (As defined in [`from`](/reference/routes/from) in your Route specification).
...
For more examples and detailed usage information, see [TCP Support](/tcp)

## Advanced Configuration

If Pomerium is listening on a port other than `443` (set with the [`address` key](/reference/address)), the `pomerium-url` flag (CLI) or "Pomerium URL" field (GUI) is required. This specifies the address and port for the client to communicate over, while the standard URL defines the port assignment for the specific route. For example:
