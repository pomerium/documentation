...
1. If you haven't already, set `generateSigningKey` as false, and set a static `signingKey` value to be shared with the Enterprise Console. See [Reference: Signing Key](/reference/signing-key) for information on generating a key:
...
[Pomerium using Helm]: k8s/helm.md
