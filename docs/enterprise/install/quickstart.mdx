...
1. If you haven't already, set `signing_key`. See the [reference page](/reference/signing-key) for more information. The same signing key must be used in both Pomerium Core and Enterprise.

   ```yaml
   signing_key: "ZZZZZZZZZZZZZZ"
   ```

1. Define the [databroker](/reference/data-broker-service) storage type and connection string. The example below assumes a local Redis server:
