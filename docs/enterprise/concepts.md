...
::: warning
Managers in any Namespace should note: while creating a route for an [upstream](/glossary.md#upstream-downstream) path prevents additional routes to that path *in the same namespace*, Managers in other namespaces can create alternate routes to the same path.
