...
// Autogenerated category index pages have no source file, so no .md sidecar is
// emitted; keep links to them as HTML instead of appending a nonexistent .md.
// Moved pages (single-sign-out, cryptography) belong in static/_redirects.
const DOC_PATHS_WITHOUT_MARKDOWN = new Set(['/docs/guides']);

const STATIC_DOC_EXTENSIONS = new Set([
  '.avif',
  '.css',
  '.gif',
  '.ico',
...
function isStaticDocAssetPath(pathname) {
  const extension = path.extname(pathname).toLowerCase();
  return STATIC_DOC_EXTENSIONS.has(extension);
}

function toCanonicalMarkdownUrl(url, {siteOrigin, routePath} = {}) {
  ...
  const origin = siteOrigin || 'https://www.pomerium.com';
  const basePathParts = routePath
    ? routePath
        .replace(/^\/+|\/+$/g, '')
        .split('/')
        .filter(Boolean)
    : [];
  if (basePathParts.length > 0) basePathParts.pop();
  const basePath =
    basePathParts.length > 0
      ? `${origin}/${basePathParts.join('/')}/`
      : `${origin}/`;
  let parsedUrl;

  try {
    parsedUrl = new URL(url, basePath);
  } catch {
    return url;
  }
  ...
  if (!isDocsOrigin || !parsedUrl.pathname.startsWith('/docs')) return url;
  if (
    parsedUrl.pathname === '/docs/api' ||
    parsedUrl.pathname.startsWith('/docs/api/')
  ) {
    return `${origin}${parsedUrl.pathname}${parsedUrl.search}${parsedUrl.hash}`;
  }
  if (isStaticDocAssetPath(parsedUrl.pathname)) return url;

  let pathname = parsedUrl.pathname.replace(/\/+$/, '');
  if (DOC_PATHS_WITHOUT_MARKDOWN.has(pathname)) {
    return `${origin}${pathname}${parsedUrl.search}${parsedUrl.hash}`;
  }

  pathname = pathname.replace(/\.(?:mdx?|html)$/, '');

  const canonicalUrl = new URL(`${pathname || '/docs'}.md`, origin);
  canonicalUrl.search = parsedUrl.search;
  canonicalUrl.hash = parsedUrl.hash;
...
function preCleanDocusaurusMDX(raw) {
  ...
  // Strip HTML comments, including comments embedded at the end of headings.
  // Loop until stable: a single pass can leave a "<!--" that removal re-forms.
  let previousRaw;
  do {
    previousRaw = raw;
 ...
 */
async function cleanMarkdownForLLM(rawContent, options = {}) {
    ...
    .use(() => (tree) => {
      ...
      visit(tree, (node, index, parent) => {
        ...
        if (node.type === 'link' || node.type === 'definition') {
          node.url = toCanonicalMarkdownUrl(node.url, options);
        }
      });
    })
    .use(remarkStringify, {
...
function buildFallbackRoutePath(relativePath, metadata = {}) {
  ...
  const routeFileName = metadata.id || fileName;

  if (
    fileName !== 'index' &&
    fileName !== 'readme' &&
    pathParts[pathParts.length - 1] !== routeFileName
  ) {
    pathParts.push(routeFileName);
  }
          ...
          try {
            ...
            try {
              cleanedContent = await cleanMarkdownForLLM(preCleanedContent, {
                siteOrigin: context.siteConfig.url,
                routePath: route.sourceRoutePath || route.path,
              });
            } catch (err) {
              throw new Error(
