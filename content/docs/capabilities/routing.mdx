...
Pomerium processes routes in the order they are defined in its configuration. If you’re using Pomerium Core, then Pomerium will source its configuration from a configuration file.

In Enterprise Console and Ingress Controller deployments, routes are defined in Pomerium’s Databroker service. In this case, Pomerium sorts routes deterministically in the following order:
