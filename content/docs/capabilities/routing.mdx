...
If your configuration includes multiple routes with identical definitions, Pomerium will process these routes in the order they are defined in your configuration file.

In the example below, if Pomerium receives a request to access `https://a.example.com:8080`, Pomerium will first check `https://a.example.com:8080/api` before matching the request with `https://a.example.com:8080`.
