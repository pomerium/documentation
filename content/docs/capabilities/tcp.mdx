...
To configure Pomerium to chain TCP connection requests, include either the `http://` or `https://` scheme in the **To** URL:
...
Specifying the `tcp://` scheme in the **To** URL instructs Pomerium to proxy the raw TCP connection to the upstream service.
