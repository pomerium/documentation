...
Navigate to the Recordings > Datasource section. Each blob datasource is scoped per-namespace.
...
Connect the instance on which you want to record sessions to Enterprise. To do so, you will need the `databroker service URL` & `shared secret` of that instance and, optionally, the certificates required to connect to it.

<Tabs>
<TabItem label="Enterprise" value="Enterprise">

Navigate to `Configure>Clusters` side panel tab, and click "Add Cluster":
...
Navigate to the Enterprise UI Section Recordings > Datasources. The UI provides drop down options for configuring the bucket URI parameters.
...
At minimum, the instance uploading the recordings will require `Storage Object User` permissions and the Enterprise instance will require the `Storage Object Viewer` permissions.
...
Session recordings can be replayed in Pomerium Enterprise. Navigate to the `Recordings > Records` side-panel tab to view available recordings. They are available by Pomerium Enterprise Namespace and Cluster, and can only be accessed by the appropriate namespace administrator.
...
| `access_type` | The type of access. `recording` : the recording contents were viewed. `metadata` : the recording metadata was viewed. `bucket`(internal) : Pomerium Enterprise made a connection to the bucket for reading. `download` : the recording was downloaded | Low |
...
The HMAC of the user id is generated from the shared_secret Pomerium Enterprise uses. Make sure when rotating shared_secrets to keep track of previous secrets for correlation on older logs.
...
S3 server access logs transform some of the raw data. For example `/`, `=` and other special characters are collected as `-` in the `user_hmac_id`. For correlation with Enterprise logs, please be mindful of this behaviour.
