...
:::tip
This example assumes you've already [created a TCP route](/docs/tcp#configure-routes) for this service.
:::

 ## Basic Connection

 1. Create a TCP tunnel, using either [`pomerium-cli`](/docs/overview/releases#pomerium-cli) or the Pomerium Desktop client. These examples use Git connections over SSH:
...
:::tip
If you've already defined an SSH configuration for all domains managed by Pomerium as described in our [SSH Example](/docs/tcp/examples/ssh#always-tunnel-through-pomerium), you can skip the first step above and define your new remote with the Git server's domain name.
:::
