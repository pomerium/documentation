...
See the "Configure Routes" section of [TCP Support](/docs/tcp#configure-routes) for more detailed information on TCP routes.
...
**Destination**: Matches the [From](/docs/enterprise/reference/manage#from) value of the route, without the protocol. Always include the port specified in the route, and do not include the `https://` protocol.
...
**Client Certificate & Certificate Key File or Text**: For routes that require client certificates for [mTLS](/docs/guides/mtls), you can provide the certificate and key file to the Pomerium Desktop client.
...
See [Release](/docs/overview/releases#pomerium-cli) to learn how to install pomerium-cli in your environment.
...
For more examples and detailed usage information, see [TCP Support](/docs/tcp)
