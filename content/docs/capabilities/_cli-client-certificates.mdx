...
<CodeBlock lang="bash">pomerium-cli {props.protocol} --client-cert cert.pem --client-key key.pem route.corp.example.com:1234</CodeBlock>

To search for a client certificate in the system trust store:

<CodeBlock lang="bash">pomerium-cli {props.protocol} --client-cert-from-store route.corp.example.com:1234</CodeBlock>
...
<CodeBlock lang="bash">pomerium-cli {props.protocol} --client-cert-from-store --client-cert-issuer "CN=My Trusted CA" route.corp.example.com:1234</CodeBlock>

Or, to filter for a certificate whose Subject contains the Organizational Unit Name "My Department":

<CodeBlock lang="bash">pomerium-cli {props.protocol} --client-cert-from-store --client-cert-subject "OU=My Department" route.corp.example.com:1234</CodeBlock>
