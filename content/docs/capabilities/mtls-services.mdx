...
Pomerium confirms the identity of an upstream service by verifying the service’s TLS certificate. (See [tls_custom_ca_file](/docs/reference/routes/tls-custom-certificate-authority) and [tls_upstream_server_name](/docs/reference/routes/tls-upstream-server-name) for more information on configuring Pomerium to accept an upstream's TLS certificate.)
...
- Run Pomerium in [all-in-one mode](/docs/internals/configuration#all-in-one-vs-split-service-mode) as a system service with a configuration file in the [standard location](/docs/deploying/from-source#configure)
...
[`tls_upstream_server_name`]: /docs/reference/routes/tls-upstream-server-name
