...
2. You should see a prompt to authenticate
...
{/* prettier-ignore */}
<iframe width="100%" height="415" src="https://www.youtube.com/embed/5Gt7u8szwac" title="Native SSH Access with Pomerium" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>

This authentication flow creates a Client binding ([See client binding](https://docs.pomerium.com/docs/internals/client-binding)) for your ssh key that is associated to your Oauth user. There is no limit on ssh keys per user, but only one user per ssh key.

### Subsequent Connections

- Run the same SSH command with your route and Pomerium server target; you will not see the code prompt or browser authentication.
- Uses cached credentials, defaults to session timeout (defaults to 14 hours; see [cookie expiration](https://docs.pomerium.com/docs/reference/cookies#cookie-expiration)) or a maximum or 48 hours. See [client binding](https://docs.pomerium.com/docs/internals/client-binding) for more info.
  - These credentials can be revoked at any time from the Pomerium UI, See [client binding](https://docs.pomerium.com/docs/internals/client-binding) for more info.
...
In versions below 0.32.0, SSH access only supports the Device Code OAuth flow. If running on those versions please make sure you consult the relevant docs for those versions.
...
Pomerium policy still applies when connecting to routes in this way, with the exception of the `ssh_username`, `ssh_username_matches_claim`, and `ssh_username_matches_email` criteria. `ssh_publickey` and `ssh_ca` still apply.
...
:::note Policy applicability on SSH

SSH routes support most general PPL criteria (`email`, `domain`, `user`, `claim`). Enterprise-only criteria supported on SSH: `groups`, `record`, `date`, `day_of_week`, `time_of_day`. Not applicable on SSH: `http_method`, `http_path`, `cors_preflight`, `client_certificate`, `invalid_client_certificate`, `pomerium_routes`, `device`.
...
There are three SSH specific runtime flags. See [Runtime Flags](/docs/reference/runtime-flags) for more information.
...
- `ssh_upstream_tunnel`: Enables the `upstream_tunnel` route configuration option. See [Reverse Tunneling](./reverse-tunneling) for details.
      ...
      - allow:
          and:
            ...
            - day_of_week:
                is: monday,tuesday,wednesday,thursday,friday
            - ssh_publickey:
                - ssh-ed25519 AAAAC...wb5aC sre1@example.com
                - ssh-ed25519 AAAAC...jtEql sre2@example.com
            - ssh_username_matches_email: true
```

Note: `groups` and `day_of_week` are Enterprise-only criteria.
