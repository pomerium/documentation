...
This authentication flow creates a Client binding ([See client binding](https://docs.pomerium.com/docs/internals/client-binding)) for your ssh key that is associated to your Oauth user. 
There is no limit on ssh keys per user, but only one user per ssh key.
...
- Uses cached credentials, defaults to session timeout (defaults to 14 hours; see [cookie expiration](https://docs.pomerium.com/docs/reference/cookies#cookie-expiration)) or a maximum or 48 hours. See [client binding](https://docs.pomerium.com/docs/internals/client-binding) for more info.
  - This credentials can be revoked at any time from the Pomerium UI, See [client binding](https://docs.pomerium.com/docs/internals/client-binding) for more info.
