...
Most SSH recording systems make you pay for audit evidence twice: once in access control, again in a bastion, host agent, or forked server that records from the side. Pomerium's model is narrower. The proxy that authenticates an SSH session also records the terminal output for routes where recording is enabled, stores the artifact in your object storage, and leaves OpenSSH on the host untouched. That matters for audit work because the recording stays attached to the same identity and access decision that allowed the session, rather than to a shared account, source IP, or sidecar recorder.

This guide is the operating companion to the [Session Recording](/docs/capabilities/session-recording) setup page. Use it when recorded interactive SSH sessions need to become defensible evidence for SOC 2 access monitoring, PCI DSS administrative-access logging, HIPAA audit controls, FedRAMP / NIST 800-53 session audit, or an internal investigation of who accessed which host and what appeared in the terminal.

Do not rely on SSH session recording alone when you need raw client keystrokes, complete command provenance, direct-command capture, SCP / SFTP transfer records, or host-level process telemetry. Pomerium records the PTY output stream, which means what the server echoed back to the user's terminal. Pair it with `sudo` I/O logging, `auditd`, EDR, or another host-side control when the audit question is what actually executed rather than what appeared on screen.

## How this approach fits

SSH evidence tools answer different questions. Host agents and eBPF integrations see deeper process activity, but require kernel-level deployment on every target. Bastions and legacy PAM session managers centralize SSH through a choke point, but often collapse users behind shared accounts or move the chain of custody into a separate runtime. Mesh and VPN recorders work when every SSH session already enters through that network. Pomerium is the right fit when the access decision already belongs at the proxy: standard SSH clients, no target-host agent, customer-owned object storage, and audit fields tied back to the authenticated user.
...
flowchart LR
  ...
  Core -. default Core cluster emits Console access logs .-> Audit[Pomerium audit logs]
  Console -. surfaces session-recording access logs .-> Audit
...
Treat that as policy surface, not a footnote. Scope recording to the routes that need evidence, decide who may enable recording and who may play recordings back, and document the limitation that input the server does not echo, such as a password at a `sudo` prompt, is not captured in the recording.
...
Pomerium's recording path is designed around write-once recording artifacts, and the Enterprise Console only reads them, for playback and download. Do not rely on application behavior alone for regulated evidence. To get a durable guarantee against a compromised credential or an insider, enable object lock and a retention policy on the bucket itself.
...
On S3, Pomerium can also specify encryption per request through the bucket URI (`ssetype=aws:kms&kmskeyid=...`); this per-request option is S3-only. Rely on the bucket default regardless, so nothing can write unencrypted.
...
The producer and the reader should use separate principals. Start from the provider permissions in the [storage reference](/docs/capabilities/session-recording/storage#provider-configuration), then reduce them with tested custom roles where your provider supports it:
...
2. Correlate. Enterprise audit entries (message `authorize blob read`) carry an `access_id`, and every entry with an `access_id` also carries the matching `user_hmac_id`. Pomerium stamps the same values onto its requests to the storage provider, so the provider log carries them too. Match on the `access_id` to tie a storage access back to a specific user action, and the `user_hmac_id` ties that action to an identity.
...
4. Confirm there are no provider data-read entries for recording content or metadata that lack Pomerium's annotation. A provider data-read entry for recording content or metadata with no matching Pomerium `access_id`/HMAC is access outside the chain of custody and should be investigated.
...
- provider data-read entries for recording content or metadata that lack Pomerium's `access_id` / HMAC annotation;
...
Use recordings as supporting evidence for access monitoring, incident investigation, and review of privileged infrastructure access. The recording, Enterprise audit log, and provider data-access log together show who accessed which system, when, and how the evidence was protected.
