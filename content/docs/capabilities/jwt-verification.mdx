...
The `issuer` and `audience` parameters are optional. If you don’t define them, `jwtVerifier` applies `firstUse` by default to the JWT provided by the identity provider. `jwtVerifier` verifies subsequent requests with these claims.

If you define the `issuer` and `audience` parameters, `jwtVerifier` verifies their values against the claims provided by the identity provider.
