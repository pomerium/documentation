...
flowchart LR
  ...
  Core -. emits audit logs .-> Audit[Pomerium audit logs]
  Console -. emits playback/download logs .-> Audit
...
2. Correlate. Enterprise audit entries (message `authorize blob read`) carry a `user_hmac_id`, and recording/download/metadata accesses also carry an `access_id`; the provider log carries the same values. Match on `access_id` where present (some operations carry only `user_hmac_id`) to tie a storage access back to a specific user action and identity. Existence checks (`HEAD` requests) are a special case: on S3 they appear in CloudTrail / access logs but carry neither the `pomerium_access_id` parameter nor the user-agent HMAC, so they cannot be correlated by those values; Azure and GCS generally do not surface existence checks in their audit logs at all.
...
4. Confirm there are no reads of the recording object that lack Pomerium's annotation. A storage read with no matching Pomerium `access_id`/HMAC is access outside the chain of custody and should be investigated. On S3, exclude `HEAD` existence checks, which legitimately carry no annotation.
