...
- [**Any Authenticated User**](/docs/reference/routes/allow-any-authenticated-user): Allows access to a route with this policy attached to any user who can authenticate to your identity provider
- [**CORS Preflight**](/docs/reference/routes/cors-preflight): Allows unauthenticated HTTP OPTIONS requests as per the CORS spec
- [**Public Access**](/docs/reference/routes/public-access): Allows complete, unrestricted access to an associated route (use this setting with caution)
