...
Users can [find their device IDs](/docs/integrations/device-context/device-identit#find-the-device-id) at the `/.pomerium` endpoint from any route.
...
Rego policies can be powerful, but improper usage may unintentionally open unauthorized access, deny valid requests, or even leak sensitive data. **Whenever possible, use [PPL](/docs/capabilities/ppl) instead**. If you’re unsure whether your use case requires Rego, work with your Pomerium account representative or [contact support](mailto:support@pomerium.com) to see if your needs can be met using PPL-based policy criteria.
