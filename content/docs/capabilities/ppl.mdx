...
If `nor` is used, the user will have access if their email address does not end in `example.com` **or** they are not a member of the admin group. **(¬A ∨ ¬B) = ¬(A ∧ B)** Warning: This operation is traditionally called NAND in the Boolean algebra.
