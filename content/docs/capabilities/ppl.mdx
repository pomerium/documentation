...
description: Learn how to use Pomerium Policy Language to build context-aware authorization policies for your services.
---
...
### Multiple Operators in an Action

You _can_ add multiple operators under the same Action block, and it would be valid PPL.

For example, this policy would grant access to anyone with an `example<X>.com` domain in their email address:
...
However, you could write an identical policy with multiple `allow` blocks:
...
Although these policies are equally effective, the second example using multiple Actions is the intended method to express PPL.
