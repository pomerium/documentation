...
1. The **transport port** the client uses to reach Pomerium. If no explicit port is set in `--pomerium-url`, `pomerium-cli` opens its HTTPS connection on TCP/443. You do not configure that on the route; it's whichever port Pomerium's [`address`](/docs/reference/address), or the edge in front of it, is exposed on.
...
The same hostname can carry multiple TCP routes for different services by varying the route-match port. Do not use `:443` in the `from` URL just to satisfy an outbound firewall rule. Use the actual service port instead, and keep the client-to-Pomerium transport on HTTPS/TCP 443.

For a complete SSH example with an L4 edge in front of Pomerium, see [SSH over TCP/443 through an L4 edge](/docs/guides/ssh-tcp-l4-passthrough).
...
- [**SSH**](./examples/ssh)
- [**SSH over TCP/443 through an L4 edge**](/docs/guides/ssh-tcp-l4-passthrough) - for environments restricted to outbound TCP/443
