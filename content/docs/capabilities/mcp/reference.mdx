...
**[Go to Blueprint: Delegate MCP Access to an LLM →](/docs/capabilities/mcp/build-mcp-chat-app)**
...
For Client routes, Pomerium exposes additional endpoints to manage MCP server discovery and upstream OAuth flows. See [Develop an MCP UI App](/docs/capabilities/mcp/develop-mcp-ui-app) for a walkthrough.
...
- [Develop an MCP UI App](/docs/capabilities/mcp/develop-mcp-ui-app) - Build ChatGPT Apps with widgets
- [Delegate MCP Access to an LLM](/docs/capabilities/mcp/build-mcp-chat-app) - Token delegation to LLM APIs
