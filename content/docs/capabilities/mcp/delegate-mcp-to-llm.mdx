...
For CI pipelines, coding agents, and background jobs where no human is present to complete an interactive login, use a Pomerium [service account](/docs/capabilities/service-accounts). A service account provides a long-lived JWT that authenticates MCP requests directly — no browser required.
...
For MCP servers that don't require upstream OAuth (i.e., internal servers behind a [basic MCP route](/docs/capabilities/mcp/protect-mcp-server)), this step is not needed.
...
See [Service Accounts](/docs/capabilities/service-accounts) for detailed setup instructions.
...
See the [Claude Code MCP documentation](https://code.claude.com/docs/en/mcp) for full configuration options.
...
See the [Codex MCP documentation](https://developers.openai.com/codex/mcp/) for full configuration options.
...
- [Service Accounts](/docs/capabilities/service-accounts) — Create and manage service accounts
