...
title: Client-Side mTLS With Pomerium
...
1. Create a wildcard certificate for `*.localhost.pomerium.io`:
   ...
   Note If you already have a certificate solution for route ingress, you can skip this step. Client certificates can be validated from a certificate authority independent of the route CA.
...
Update the `config.yaml` file or environment variables with the following additions. Replace `/YOUR/MKCERT/CAROOT` in this example with the value of `mkcert -CAROOT`:

:::tip

Configuration will require client certificates for _all_ routes. See [require mTLS per Route](#require-mtls-per-route) to enable mTLS on for specific routes.
