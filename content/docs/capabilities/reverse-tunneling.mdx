...
This uses the [Native SSH Access](./native-ssh-access) feature, and works best
with the standard OpenSSH client.
...
1. Follow steps 1-3 in
   [Native SSH Access - Setup](/docs/capabilities/native-ssh-access#setup) to
   configure the SSH address, host keys, and user CA key. Note that SSH routes
   are not necessary to be able to use this feature, so steps 4 and 5 can be
   skipped.
...
Reverse tunneling is only enabled when connecting to Pomerium without a route,
the same way as the Internal CLI is accessed (see
[Using the Internal CLI](./native-ssh-access#using-the-internal-cli)).
...
When connecting to a SSH route directly, any port-forwarding requests are sent
directly to the upstream SSH server, and are not interpreted by Pomerium. For
example, the following command does _not_ enable reverse tunneling:
...
to control the destination of an incoming request. <b>The local address is not
sent to the server.</b>
...
When connecting to Pomerium as a reverse tunnel upstream, the tunnel status TUI
...
The Active Connections panel displays live connections through the reverse
tunnel. For HTTP routes, the hostname and path in the request will be displayed.
For SSH routes, the route name will be displayed.

The downstream client IP is also displayed in this panel, along with bytes sent
through the tunnel in both directions (Rx = downstream to upstream, Tx =
upstream to downstream), and the duration of the connection. Connection stats
are updated periodically while the connection is open, and immediately when the
...
with <span style={{color: "blue"}}>`D`</span>, for example:
![TUI All Routes](./img/ssh/tui/out/tui_all_routes.png)

:::tip

Dynamic port numbers are known to the SSH client. They can be used to identify
specific requests to deactivate them at runtime using the escape console. See
[OpenSSH Escape Commands](#openssh-escape-commands).
...
requests. If it is, it will display <span
...
is either <span style={{color: "green"}}>`HEALTHY`</span>, <span
style={{color: "orange"}}>`DEGRADED`</span>, or <span
style={{color: "red"}}>`UNHEALTHY`</span> (see
[Envoy docs](https://www.envoyproxy.io/docs/envoy/latest/intro/arch_overview/upstream/health_checking)).
If the route is currently failing its health checks, it will show <span
style={{color: 'gray'}}>`INACTIVE`</span> in the `Status` column.
...
The TUI has full mouse support for terminals which support it (all modern
terminals do).

Some features, such as the ability to copy to the clipboard, are not supported
by all terminals. If your terminal supports this feature (see
[here](https://github.com/contour-terminal/vt-extensions/blob/master/clipboard-extension.md)
for an unofficial support matrix), you will have the ability to copy URLs to the
clipboard in the right-click context menu of some table rows.
...
- The SSH client enables dynamic port-forwarding when the local `host:port` half
  of the `-R` argument is omitted.

If the remote port is 0, but a local host:port is _also_ set, Pomerium will be
expecting dynamic port-forwarding, but the local ssh client will not be. This
will cause downstream clients (including internal health checks) to time
out.<b>[^5]</b> The TUI will also display a warning with hints on how to fix the
command:

`ssh -R https-*:0:localhost:8080 example.com`
![TUI Warning 1](./img/ssh/tui/out/tui_warning_logs_static.png)

If the remote port is not 0, and the local host:port is omitted, then the local
ssh client will be expecting dynamic port-forwarding, but Pomerium will not be.
This will result in immediate downstream errors (not timeouts). The TUI will
also display a similar warning:

`ssh -R https-route.example.com:443 example.com`
![TUI Warning 2](./img/ssh/tui/out/tui_warning_logs_dynamic.png)
...
[Wildcard From Route](/docs/reference/routes/from#wildcard-from-routes) of the
...
[^5]:
    Depending on the upstream server and protocol, the channel may or may not be
    held open until the timeout. It is normal to see `CLOSED` status with a
    short duration in the TUI, yet the downstream client is stuck waiting. You
    may also see a channel with `OPEN` status, which transitions to `CLOSED` at
