...
Any production deployment with more than one instance of Pomerium (in any combination of modes) should be configured to use Postgres as the [`databroker_storage_type`](/docs/reference/data-broker-storage-type). See [Data Storage - Backends](/docs/internals/data-storage#backends) for more details.
...
Our [Kubernetes](/docs/kubernetes/quickstart) supports [Horizontal Pod Autoscaling](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/).
...
Note that deployments on Kubernetes can utilize The [Pomerium Ingress Controller](/docs/kubernetes/ingress) to simplify configuration.
