...
- Your application backend can [list MCP servers that are available](#listing-available-mcp-servers) on the Pomerium backend and initiate upstream OAuth flows if necessary.
...
For Client routes, Pomerium exposes additional endpoints to manage MCP server discovery and upstream OAuth flows.
