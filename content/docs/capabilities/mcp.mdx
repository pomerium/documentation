...
Proxy an internal MCP server through Pomerium so MCP clients can access it securely — no auth logic needed on the server side.
...
### [Develop an MCP App](/docs/capabilities/mcp/develop-mcp-app)
