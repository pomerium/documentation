...
The [Pomerium Ingress Controller](https://github.com/pomerium/ingress-controller) includes experimental support for the Kubernetes [Gateway API].
...
If you are already using the Gateway API to model service networking in Kubernetes or are interested in using it for the first time with Pomerium, you can use the Kubernetes Gateway API with the Pomerium Ingress Controller to manage supported ingress routes.

To learn more about the Gateway API in general, see the official [Gateway API documentation][gateway api].

## Supported features

Pomerium now includes partial support for "Core" features from the Gateway API v1.2, for the "Gateway" and "HTTP" conformance profiles.

Some features are not supported:

- Gateway listeners must use the HTTPS protocol, not HTTP.

  (Plaintext HTTP is not supported because Pomerium uses session cookies for authentication, and it is not safe to transmit session cookies over unencrypted HTTP.)
...
Pomerium defines a PolicyFilter CRD for setting the authorization policy for a particular route. Policies are expressed using [Pomerium Policy Language](/docs/capabilities/ppl) (PPL).
...
To install the Pomerium Ingress Controller and enable Gateway API support:
...
1. Then install the Pomerium Ingress Controller:
...
1. We'll need a TLS certificate for the domain to be used with this route. (For testing purposes you could generate a certificate using [mkcert].)
