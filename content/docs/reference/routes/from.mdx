...
In v0.26, we've updated the default route matching behavior so that From routes defined _without_ a specific port in the route URL will match a request on any port.
...
With a From route like the one below...

```yaml
from: https://www.example.com
```

...Pomerium would match the incoming requests on any port:
...
To enable [TCP proxying](/docs/capabilities/tcp/) support, use the `tcp+https` scheme in the From URL.
