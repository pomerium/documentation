...
Specifying `tcp+https` or `udp+https` for the scheme enables [TCP proxying](/docs/capabilities/tcp/) or [UDP proxying](/docs/capabilities/udp) (available since v0.29) support for the route. You may map more than one port through the same hostname by specifying a different `:port` in the URL.
