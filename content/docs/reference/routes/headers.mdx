...
<EnterpriseConfiguration name="host_rewrite" resource="route">
...
### Original Host Header \{#original-host-header}

When host rewrite is active, Envoy appends the original `Host`/`:authority` value to the standard `x-forwarded-host` header. This applies to the default `auto_host_rewrite` behavior as well as explicit `host_rewrite`, `host_rewrite_header`, and `host_path_regex_rewrite` settings.

This header is not appended when [`preserve_host_header`](#1-preserve-host-header) is enabled, because the `Host` header is not being rewritten.

The forwarded value matches the client's original `Host` exactly, including port if present (for example, `app.example.com:8443`). If `x-forwarded-host` is already present, Envoy appends the original host value when it differs from the last value already in the header.
...
<TabItem value="Enterprise" label="Enterprise">
<EnterpriseConfiguration name="set_request_headers" resource="route">
...
<TabItem value="Enterprise" label="Enterprise">
<EnterpriseConfiguration name="remove_request_headers" resource="route">
...
<TabItem value="Enterprise" label="Enterprise">
<EnterpriseConfiguration name="set_response_headers" resource="route">
...
<TabItem value="Enterprise" label="Enterprise">
<EnterpriseConfiguration name="rewrite_response_headers" resource="route">
