...
| **Parameter name** | **Type** | **Usage**    | **Default** |
| :----------------- | :------- | :----------- | :---------- |
| `idle_timeout`     | String   | **optional** | `5m`        |

### Examples

```yaml
resource "pomerium_route" "verify_route" { name         = "verify-route" namespace_id = pomerium_namespace.test_namespace.id from         = "https://verify.localhost.pomerium.io" to           = ["https://verify.pomerium.com"] policies     = [pomerium_policy.test_policy.id] idle_timeout = 5m }
```
