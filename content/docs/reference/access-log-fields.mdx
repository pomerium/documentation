...
| `forwarded-for` | Identifies the originating IP address of the client (this field is the value of the [`X-Forwarded-For`](/docs/reference/x-forwarded-for-http-header) header as sent by the upstream service) | Yes |
...
| `response-code-details` | The HTTP Response Code Details set by Envoy (for example, `File not found` or `404`; see [Envoy docs](https://www.envoyproxy.io/docs/envoy/latest/configuration/http/http_conn_man/response_code_details) for more information) | Yes |
