...
| **[Parameter name](/docs/k8s/reference#spec)** | **Type** | **Default** |
| :-- | :-- | :-- |
...
| `forwarded-for` | This is the value of the [`X-Forwarded-For`](/docs/reference/x-forwarded-for-settings#x-forwarded-for-http-header) header (as sent to the upstream service) | Yes |
| `ip` | The user's IP address. Note that this depends on setting the [`xff_num_trusted_hops`](/docs/reference/x-forwarded-for-settings#the-number-of-trusted-hops) option appropriately. | No |
