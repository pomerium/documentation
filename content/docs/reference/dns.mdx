...
If `dns_lookup_family` is unset, Pomerium applies `V4_PREFERRED`. There is no `DEFAULT` config value: the **Default** entry in the Enterprise Console's DNS Lookup Family dropdown leaves the setting unset, with the same effect.
