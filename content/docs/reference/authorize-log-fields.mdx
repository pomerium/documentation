...
| `host` | The HTTP Authority. Can be a hostname, domain name, or IP address and may contain a port number (for example, `www.example.com`) | Yes |
...
To find authorize logs, in your terminal search for `"message":"authorize check"`:
...
If `authorize_log_fields` is not defined, all the available default fields will be displayed.
