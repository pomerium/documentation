...
- **Upstream Request Success/Failure** – `upstream_rq_total` counts requests Envoy has sent to each cluster. Success and error rates can be inferred from response-code-specific metrics (if tagging is enabled, you may see `envoy_cluster_upstream_rq_xx` with code labels). In many setups Envoy provides counters for HTTP status classes per cluster - e.g., `envoy.cluster.<cluster_name>.upstream_rq_2xx/5xx`. These indicate how many requests to the cluster succeeded or failed (server error). Monitoring a rising rate of `upstream_rq_5xx` or `upstream_rq_timeout` (see below) on a cluster is critical for detecting backend issues.
- **Upstream Request Latency** - `upstream_rq_time` is a histogram of time spent on an upstream request (including waiting and response) in milliseconds. High quantiles here mean the backend is responding slowly. This metric can be broken down per cluster.
- **Active/Pending Upstream Requests** – `upstream_rq_active` is a gauge of current in-flight requests to a given cluster. `upstream_rq_pending_active` is a gauge of requests queued waiting for a connection (if connection pooling is exhausted). Spikes in pending requests suggest the upstream or Envoy's connection pool is saturated and could indicate bottlenecks.
- **Upstream Errors** – Key counters for failure modes include `upstream_rq_timeout` (requests to upstream that timed out with no response), `upstream_cx_none_healthy` (connection attempts failed due to no healthy hosts), and `upstream_rq_per_try_timeout` (per-retry timeouts). These should normally be zero; any significant values indicate upstream problems (e.g., `upstream_cx_none_healthy` means Envoy couldn't even find a healthy endpoint to connect).
- **Retry/Circuit-breaker Metrics** – If Envoy is retrying requests or hitting circuit breaker limits, metrics like `upstream_rq_retry`, `upstream_rq_retry_overflow` (retry budget exceeded), or circuit breaker gauges under `circuit_breakers` (e.g., `remaining_cx`, `remaining_rq`) can be monitored. These are more advanced but can hint at load shedding. For example, Envoy exposes `cluster.<name>.circuit_breakers.default.cx_open` (indicator if connection limit reached) and `rq_open` for request limits - a value of `1` means that circuit breaker is tripped (no more connections or requests will be allowed). You might monitor `remaining_cx`, `remaining_rq` etc., which tell how many resources remain before hitting limits.
