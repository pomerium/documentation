...
Each databroker instance should have the same shared secret, but its own node ID and its own storage backend. It is recommended to use the `file` storage backend. If `postgres` is used, the instances should **not** share the same Postgres database.
