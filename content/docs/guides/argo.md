...
This is a Kubernetes guide: Argo Workflows and the Pomerium Ingress Controller run on a cluster and cannot be exercised in the sealed Docker validation harness, so `content/examples/guides/argo/validate/SKIP` marks it accordingly. The committed `Ingress` manifest (`content/examples/guides/argo/argo-ingress.yaml`) was validated offline with `kubectl apply --dry-run=client`, and the Argo Server service name and port (`argo-argo-workflows-server:2746`) were confirmed by rendering the `argo/argo-workflows` Helm chart with `helm template`. The end-to-end login flow above was reviewed against the current Pomerium Ingress Controller behavior rather than run in CI.
