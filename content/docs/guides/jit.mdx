...
Replace the `REPLACE_WITH_*` values with your own, then start it:
...
3. **An expired grant is denied.** Once the `before` time has passed, the same authenticated user gets a 403 forbidden page with no change to the policy.
4. **Only admins approve.** A non-admin who reaches `/admin` is denied by that route's policy.

## Validation

This guide is not covered by the repository's sealed end-to-end harness. That harness runs open-source Pomerium Core from a config file, and the `date` matcher is implemented in the Pomerium Zero/Enterprise policy engine, so Core rejects it at startup (`unknown policy criterion: date`). The time-window behavior was instead verified manually against a Pomerium Zero cluster: a grant with a future `before` date allowed the user, and the same grant with a past `before` date returned a 403 for the still-authenticated user. The `jit-example` automation was verified to create and update the cluster policy through the Zero API. See `content/examples/guides/jit/validate/SKIP` for the recorded reason.
