...
3. On the route, enable **Pass Identity Headers** so Pomerium forwards identity to Open WebUI, and enable **Allow WebSockets** so streamed responses work.
...
The compose file uses floating image tags (`pomerium/pomerium:latest`, `open-webui:main`) so the example stays current. For production, pin both to a specific version tag or digest so deployments are reproducible.
...
Trusted-header SSO means Open WebUI believes whatever identity arrives in `X-Pomerium-Claim-Email`. That is safe only when Pomerium is the sole path to the app. If a user can reach Open WebUI directly, they can forge that header and impersonate anyone.

Keep Open WebUI off published host ports (the compose file only exposes Pomerium), put both services on a private network, and make sure no firewall, load balancer, or other route bypasses Pomerium to hit `8080`. Treat any direct path to the upstream as an authentication bypass.
