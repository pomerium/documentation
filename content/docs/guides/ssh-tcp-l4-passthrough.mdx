...
keywords:
  [pomerium, ssh, tcp, tcp routes, http connect, pomerium-cli, l4 load balancer, nginx stream]
---
...
Check the Pomerium authorization log. This shows the CONNECT authority Pomerium used to match the route and that the demo policy admitted the tunnel:
...
For log field definitions, see [Authorize Log Fields](/docs/reference/authorize-log-fields).
