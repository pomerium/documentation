...
| The upstream returns `401`, and no `secret` log line ever appears | The image predates secret injection, so the reference expanded to an empty string | `docker compose logs pomerium \| grep 'unknown config option'` | Use an image that includes the feature. Without it there is no fail-closed behavior: the header is sent empty |
