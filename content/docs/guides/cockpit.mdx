...
This guide ships a sealed end-to-end test (`content/examples/guides/cockpit/validate/`) that runs checks 1 and 2 against an in-network identity provider on every change. It stops at Cockpit's login screen on purpose, because the host login is Cockpit's own concern, not Pomerium's.
