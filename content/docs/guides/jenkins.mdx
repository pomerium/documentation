...
This guide uses Auth0 as the IdP and imports an **admin** group configured in Auth0's dashboard. Complete the [Auth0 guide](/docs/identity-providers/auth0) to follow along with this guide, or import groups from your IdP.
...
3. In the **Criteria** field, select **Claim**
4. In the **Type** field, enter the sub-path (optional)
5. In the **Value** field, enter the criterion value

![Create Jenkins policy](./img/jenkins/8-enterprise-jenkins-policy.png)
...
- **JWKS JSON URL** appends `/.well-known/pomerium/jwks.json` to the authenticate service URL. The JWKS endpoint provides Jenkins the user’s public key to verify their JWT signature.
