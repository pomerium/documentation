...
In this guide, you'll secure an instance of Jenkins behind Pomerium Zero using single sign-on (SSO) and custom JWT claims.
...
If you haven't, install Java and Jenkins on your Linux system. Then, check the status of your Jenkins instance:

```shell-session
$ sudo systemctl status jenkins
● jenkins.service - Jenkins Continuous Integration Server
     Loaded: loaded (/lib/systemd/system/jenkins.service; enabled; preset: enabled)
     Active: active (running) since Wed 2024-04-10 13:42:10 UTC; 26s ago
   Main PID: 385 (java)
      Tasks: 51 (limit: 4686)
     Memory: 1.1G
        CPU: 51.586s
     CGroup: /system.slice/jenkins.service
             └─385 /usr/bin/java -Djava.awt.headless=true -jar /usr/share/java/jenkins.war --webroot=/var/cach>
```
...
First, build a simple policy. The policy built below will only grant access to a request if the user's email address contains the specified domain:
...
1. Enter the domain portion of the email address you'll use to access Jenkins (for example, `@example.com`)
1. Save your policy
...
After you've saved your route and policy, **apply the changeset** so Pomerium applies these changes to your cluster configuration:
...
1. You can also give **Anonymous** users **Overall Read** privileges. This way, if you access Jenkins from another account, the user won't be able to make any changes to the instance.
