...
lang: en-US
keywords: [pomerium, ssh, zero, native ssh, oidc, identity aware proxy, ssh certificates]
...
Both fields appear in the Configure Global SSH Settings step shown above. Paste the private keys, not the `.pub` files, into the host-key and CA fields. The Console has a **How to generate SSH keys?** helper if you need the commands again. Select **Next** to move on to the route.
