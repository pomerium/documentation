...
To use your own domain instead, follow [adding a custom domain in Zero](https://www.pomerium.com/docs/get-started/fundamentals/zero/zero-custom-idp#next-steps-add-a-custom-domain). A common convention is an `ssh.` prefix, so the command becomes `ssh myuser@myroute@ssh.company.com`.
...
This guide is driven entirely through the managed Zero Console and an interactive OAuth login during the SSH handshake, so it can't run in the repository's sealed Docker validation harness. The flow above was verified manually against a Zero cluster. The underlying native SSH capability is documented and tested separately on the [Native SSH Access](/docs/capabilities/native-ssh-access) page.
