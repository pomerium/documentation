---
# cSpell:ignore homelab talosctl talconfig metallb kubeseal ddns mikrotik beelink minisforum argocd gitops siderolabs controlplane talosconfig kubeconfig ipaddresspool ipaddresspools ipify checkip jsonencode parentbased traceidratio dogfood dogfooding
...
Use the latest stable Talos release when you build the cluster. The [live-lab snapshot](#14-current-live-lab-snapshot) at the end of this guide shows the real cluster state this example is based on.
...
The live lab snapshot later in this guide still shows Argo CD `v3.1.0`. The install example uses `v3.3.6` on purpose so a fresh deployment starts from a newer reviewed release instead of inheriting the lab's upgrade lag.
...
Only disable Argo CD's built-in auth if the UI is reachable exclusively through a correctly protected Pomerium route. If the `from` URL, attached policy, or ingress path is wrong, anything that can reach the service endpoint can reach the Argo CD UI without its own login prompt.
...
You should see MetalLB and Argo CD running before you move on to workload deployment.
...
If a token such as the Cloudflare DDNS API credential expires, rotate the plain Secret source, reseal it, commit the updated sealed manifest, and let Argo CD reconcile the replacement.
...
- `kubectl -n pomerium get pods,svc`
- `kubectl -n pomerium logs deploy/pomerium --tail=20`

You should see the Pomerium pod become `Running` and the Service receive the MetalLB address you reserved for the edge.
...
Treat Terraform rollback as its own workflow. Argo CD's `selfHeal` will revert Kubernetes drift, but it will not undo route or policy changes in Pomerium Zero for you.

For route and policy mistakes:

- review the plan before apply
- revert the HCL in Git if you need to back out a change
- re-run `terraform plan` and `terraform apply` to return Pomerium Zero to the previous state
- reserve `terraform destroy` for disposable environments, not your primary homelab

If you use Terraform Cloud on the free tier, expect queued plans when another run is already in progress.
...
- confirm the Terraform run planned or applied successfully
- `curl -I https://verify.example.lab`

Before you continue, you should be able to get a `302` redirect from a protected route instead of a missing-route or upstream-failure response.
...
Unlike the other components in this guide, `pomerium/verify` publishes only rolling tags (`latest`, `main`, `sha-*`), so it is intentionally not version-pinned. It is a throwaway test app, not part of your real workload set.
...
- a user your policy excludes is sent to sign in and then denied, never reaching the upstream
...
Confirm the Grafana auth-proxy environment variables against the Grafana version you deploy. Auth proxy settings move over time, but the core requirement does not: only trust headers from the Pomerium-side network path you control.
...
Pin the chart version you have actually reviewed. Do not leave this on a floating semver range if the rest of the stack is supposed to move intentionally.
...
Use the same failure model every time: identify the symptom, confirm which system owns the broken state, then check the smallest number of things that can prove or disprove that theory.

| Symptom | Likely cause | What to check | Fix |
| --- | --- | --- | --- |
| `504` after a Terraform route change | Argo CD has not synced the Service or Deployment yet | `kubectl get pods -n verify`, `kubectl get applications -n argocd` | Wait for Argo CD to finish, then re-test the route |
| Pomerium pod crashes on start | `POMERIUM_ZERO_TOKEN` is missing or invalid | `kubectl logs -n pomerium deploy/pomerium` | Recreate or reseal the token secret and let Argo CD sync it |
| Argo CD stays `OutOfSync` after a Terraform run | Terraform and Argo CD are crossing ownership boundaries | Check whether Terraform is trying to manage Kubernetes objects | Move Kubernetes ownership back into Argo CD and keep Terraform on the Zero side only |
| `302` loops on a protected route | `authenticate_service_url`, route hostnames, or Argo CD OIDC issuer do not match | Check `authn.example.lab` reachability and the configured issuer URLs | Align the Pomerium authenticate URL, route `from` values, and OIDC issuer settings |
| MetalLB never assigns an ingress IP | `L2Advertisement` or pool config is missing or wrong | `kubectl describe svc pomerium -n pomerium`, `kubectl get ipaddresspools,l2advertisements -n metallb-system` | Apply or fix the MetalLB pool and advertisement manifests |

## 13. Validate every change the same way

:::note Verify against your own cluster

This is a multi-component infrastructure guide (bare-metal Talos Kubernetes, Terraform Cloud, Argo CD, and a hosted Pomerium Zero control plane). There is no single command that proves the whole pipeline, so verify each layer against your own cluster using the checks below and the per-section checkpoints. The install versions here are normalized for a fresh deployment and differ from the [live-lab snapshot](#14-current-live-lab-snapshot), so treat them as a reviewed starting point rather than an exact replica.
...
Run this against each protected route -- `verify`, `grafana`, and `argocd` -- and confirm every one returns a `302` redirect to the authenticate service while its Argo CD application stays `Synced` and `Healthy`.

## 14. Current live lab snapshot

This example is based on a real lab. At the time of writing, that cluster was running:
...
The snippets above are not a byte-for-byte export of that repo. Where the live lab carries migration or upgrade lag, the guide uses a newer reviewed version instead -- for example Argo CD `v3.3.6` in the install step versus the lab's running `v3.1.0`, and a stable Pomerium image instead of the lab's dogfood build.
