...
This guide ships a sealed end-to-end test (`content/examples/guides/hedgedoc/validate/`) that runs the first two checks against an in-network identity provider on every change.
...
- **Certificate errors reaching the route.** Make sure DNS for `hedgedoc.yourdomain.com` points at Pomerium and that `autocert` could reach Let's Encrypt on ports 80/443.
...
The Compose file above does not set `CMD_ALLOW_ANONYMOUS`, so HedgeDoc keeps its own login and registration in front of note creation even after Pomerium lets a user through. The route policy is your outer perimeter and HedgeDoc's own settings control what an authenticated user can do once inside. If you set `CMD_ALLOW_ANONYMOUS=true`, anyone your Pomerium policy admits can use HedgeDoc with no HedgeDoc account, so scope the route policy accordingly. The demo `database` password is `password`; change it before running anything beyond a local demo.
