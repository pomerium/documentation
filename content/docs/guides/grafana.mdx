...
### Configure Grafana for JWT authentication
...
In `jwk_set_url`, replace `auth.localhost.pomerium.io` with the value of [`authenticate_service_url`] in your Pomerium configuration. See the Pomerium Core [quick-start] guide for more information.
...
- `header_name = X-Pomerium-Jwt-Assertion` tells Grafana which HTTP header to look for the JWT (see [Identity Verification] for more information)
...
1. Under the shield icon in the main menu (**Server Admin**), select **Users**.
   ...
   :::caution This is distinct from the **Users** option under the cog wheel (**Configuration**), which will only finalize a new user when they accept an invite via email or link. :::
...
1. Click the **New user** button to create a new user. Make sure that the email address matches the one provided by Pomerium via your IdP.
   ...
   :::tip You can access the special endpoint `/.pomerium` from any Pomerium route to view the data provided by Pomerium in the JWT. :::
...
Note Docker-based installations may need to be restarted to pick up the new route.
