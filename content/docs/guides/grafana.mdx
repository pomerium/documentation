...
- `header_name = X-Pomerium-Jwt-Assertion`: tells Grafana which HTTP header to look for the JWT (see [Identity Verification] for more infor
mation)
 `header_name = X-Pomerium-Jwt-Assertion`: tells Grafana which HTTP header to look at for the JWT (see [Identity Verification] for more in
formation)
