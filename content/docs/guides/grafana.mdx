...
You'll put Grafana behind Pomerium so that Pomerium handles single sign-on and authorization, then forwards a cryptographically signed identity JWT to Grafana. Grafana verifies that JWT and signs the user in automatically, so there's no second login prompt or separate Grafana password for users.
...
3. **JWT SSO works.** Grafana signs you in automatically. Open `/profile` and confirm your name, email, and username show **Synced via JWT**.
   ...
   ![Grafana profile page with Name, Email, and Username marked "Synced via JWT"](./img/grafana/grafana-profile.png)
