...
You'll also instruct Pomerium to [rewrite the host header](http://localhost:3002/docs/reference/routes/headers#host-rewrite) value to avoid a [`403: Origin not allowed`](https://github.com/grafana/grafana/issues/45117#issuecomment-1033842787) error.

1. Select the **Headers** tab
1. Apply **Pass Identity Headers**
1. In **Host Rewrite to Header**, enter the route’s external URL without the protocol
1. Save the route and apply your changeset
    ![Configuring the headers settings for the Grafana route in the Zero Console](./img/grafana/grafana-headers-settings.gif)
...
Add the Grafana configuration below to the Docker Compose file running Pomerium Zero. The Grafana environment variables enable Grafana to accept JWTs from the incoming request and what claims to look for.
...
In lines `14`, `22`, and `27`:

- Replace `CLUSTER_STARTER_SUBDOMAIN` with your cluster's starter subdomain. For example, if your starter domain is `curious-cat-9999.pomerium.app`, lines `14` and `27` would be `authenticate.curious-cat-9999.pomerium.app`.
...
Save your changes and apply your changeset.
...
[ingress-cert-manager]: /docs/deploy/k8s/ingress.md#cert-manager-integration
