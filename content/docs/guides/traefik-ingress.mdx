...
Before You Proceed Since this guide was written, Pomerium has developed into a first-class proxy service, and provides an integrated [Ingress Controller](/docs/k8s/ingress) for Kubernetes.
...
:::tip Please note `forwardedHeaders.insecure` must be set on the entrypoint in front of Pomerium proxy if you are routing forward auth requests through Traefik. See [docs](https://docs.traefik.io/routing/entrypoints/#forwarded-headers) for more information. :::

Middleware:

<Middleware />

:::caution Please note `trustForwardHeader: true` must be set for the middleware to work correctly. See [docs](https://docs.traefik.io/middlewares/forwardauth/#trustforwardheader) for more information. :::
...
<iframe
  width="100%"
  height="500"
  src="https://www.youtube.com/embed/wrvNV9fP5hw?rel=0"
  frameborder="0"
