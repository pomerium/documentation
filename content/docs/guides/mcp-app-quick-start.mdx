...
Register the widget resource on the server with the correct MIME type:
...
The `RESOURCE_MIME_TYPE` constant is `text/html;profile=mcp-app`, required for MCP hosts to recognize and render the widget.
...
Use the same identity provider for both Pomerium and your MCP host. If you're seeing auth errors, confirm you're signed in to both with the same account.
