...
3. **Header auth signs you in.** You land on the Guacamole home screen as your own user, not the Guacamole username/password form.
   ...
   To confirm it's the header (not a form login) that authenticated you, mint a token through Guacamole's REST API and check the response. `username` is your forwarded email and `dataSource` is `header`:

   ```bash
   curl -s -X POST https://guacamole.yourdomain.com/api/tokens
...
The whole setup rests on two properties. Both are enforced by the Compose file above, and both must hold in production.
