...
We'll use [Linear's](https://linear.app) remote MCP server (`https://mcp.linear.app/mcp`) for this guide so the steps, policies, and screenshots stay concrete. You'll stand up a Pomerium Zero route in front of the Linear remote MCP server, then use your existing identity provider to gate who can connect, which workspace their tokens come from, and which tools the agent is allowed to call.
