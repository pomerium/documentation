...
- **A persistent databroker backend** configured on the cluster. The in-memory default is not suitable for MCP bridging (see [What state lives in the databroker](#what-state-lives-in-the-databroker) below). Use the `file` (Pebble-backed) backend for single-replica deployments or `postgres` for multi-replica; set the connection string in the Zero Console under **Settings → General**. See [Databroker Storage Settings](/docs/reference/databroker) and [Persistence & Data storage](/docs/internals/data-storage).
...
### What state lives in the databroker \{#what-state-lives-in-the-databroker}

Most of what makes the bridge "feel seamless" on the second and third connection is persisted in the [databroker](/docs/reference/databroker):

- Pomerium's [Client ID Metadata Document (CIMD)](/docs/capabilities/mcp/mcp-upstream-oauth#auto-discovery-rfc-9728) client registration with Linear's authorization server
- Per-user upstream access token (TI) and refresh token
- Pomerium-issued MCP client credentials (TE) and session records

If the databroker is running with the default in-memory backend and Pomerium restarts, all of this disappears: every user has to redo Linear consent, and Pomerium has to re-register via CIMD. In a multi-replica deployment, the in-memory backend also means each replica has its own view, so a token minted on replica A won't be recognized on replica B. That's why the prerequisites list a persistent backend: `file` (Pebble-backed) for single-replica, or `postgres` for multi-replica.
...
| --- | --- | --- |
| **Settings → MCP** (cluster-wide) | Server-Side Request Forgery (SSRF) allowlists for OAuth metadata discovery (`mcp_allowed_as_metadata_domains`, `mcp_allowed_client_id_domains`) | Step 1 |
| **Route editor → Advanced tab → MCP panel** (per route) | Route-level MCP properties (server vs. client mode, pre-registered upstream credentials) | Step 2 (read-only for this guide) |
...
If you see the IdP sign-in but Linear's consent screen never appears, the route isn't actually saved as an MCP server route. Re-open the route in the Zero Console and confirm **Route Type: MCP** on the General tab and an `mcp: { server: {} }` block in the PPL Editor. A route saved without Route Type MCP will proxy to Linear but won't trigger the bridging handshake. The other common cause is `mcp.linear.app` missing from `mcp_allowed_as_metadata_domains` (step 1).
...
| After a Pomerium restart, every user is asked to re-authorize Linear | Databroker is configured with the in-memory backend (or its storage has been wiped), so the cached TI tokens and CIMD registration are gone | Zero Console **Settings → General → Databroker Storage Connection String**; verify a `file://` or `postgres://` value is set | Switch to `file:///var/lib/pomerium` (Pebble-backed) for single-replica or a Postgres connection string for multi-replica. See [Databroker Storage Settings](/docs/reference/databroker). |
| Works for users against one replica, fails with "not connected" on another | Multi-replica cluster running with the `file` or `memory` backend, so replicas don't share state | Zero Console **Reports → Status** (replica count); **Settings → General → Databroker Storage Connection String** | Move the cluster to a Postgres backend so all replicas share one databroker; see [Databroker Storage Settings](/docs/reference/databroker). |
