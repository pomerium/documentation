---
title: 'Bridge a Third-Party MCP Server'
...
| Let a large language model (LLM) backend (OpenAI, Anthropic) call MCP servers on a user's behalf | [Delegate MCP Access to an LLM](/docs/capabilities/mcp/delegate-mcp-to-llm) |
...
- **Authentication configured.** Pomerium Zero's default hosted authenticate works out of the box. You can also [bring your own IdP](/docs/get-started/fundamentals/zero/zero-custom-idp) if needed.
...
sequenceDiagram
  ...
  U ->> C: Adds https://linear.slyfox-1234.pomerium.app/mcp
...
| Name                  | `Linear MCP`                                     |
| From → Host           | `linear.slyfox-1234.pomerium.app`                |
...
![Route editor General tab for the Linear MCP route, showing Route Type MCP, From linear.slyfox-1234.pomerium.app, To https://mcp.linear.app, and the Pomerium Team policy attached](./img/mcp-bridge/route-linear-mcp-general.png)
...
![MCP Inspector connection form with Transport Type set to Streamable HTTP and URL set to https://linear.slyfox-1234.pomerium.app/mcp, ready to click Connect](./img/mcp-bridge/inspector-connect-form.png)
...
This flow depends on a managed Pomerium Zero control plane, Linear's hosted MCP server, and interactive OAuth consent from a real MCP client. Confirm it end to end by running the four checkpoints above against your own cluster, including the `issue_delete` -> `403` deny check in Checkpoint 4.
