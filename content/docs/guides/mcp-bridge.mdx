...
After you add tool restrictions from [Limit MCP Tool Calling](/docs/capabilities/mcp/limit-mcp-tools), repeat this checkpoint with a blocked tool. A denied `issue_delete` call should show `"allow-why-false": ["mcp-tool-unauthorized"]` and Pomerium should return `403` to the client.
