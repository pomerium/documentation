...
Pomerium gates the route, and GitLab runs its own login on top. Checkpoints 1 and 2 are an auth and upstream smoke test: they confirm Pomerium requires sign-in and that an allowed user reaches GitLab's sign-in page. GitLab's own root login and first-run setup (checkpoint 3) are verified manually, since that flow is GitLab's, not Pomerium's.
