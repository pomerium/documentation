...
lang: en-US
image: ./img/gitlab/gitlab-dashboard.png
...
```mermaid
flowchart LR
    Browser --> Pomerium["Pomerium<br/>front-door authN + authZ"]
    Pomerium -.->|"sign in"| IdP[Identity provider]
    Pomerium --> GitLab["GitLab<br/>own login on top"]
```
...
4. **A disallowed user is blocked.** Sign in as a user your policy excludes and open `https://gitlab.yourdomain.com`. Pomerium denies access, so the request never reaches GitLab's sign-in page.
