...
Pomerium gates the route, and GitLab runs its own login on top. Checkpoints 1 and 2 confirm that Pomerium requires sign-in and that an allowed user reaches GitLab through the gate; checkpoint 3 confirms GitLab itself is reachable. GitLab's root login and first-run setup are GitLab's own concern, not Pomerium's.
