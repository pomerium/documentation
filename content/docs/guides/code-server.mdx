...
Because code-server has an [unwieldy password authentication process](https://coder.com/docs/code-server/latest/guide#expose-code-server), devteams use Pomerium to add [authentication](https://www.pomerium.com/docs/capabilities/authentication) and [authorization](https://www.pomerium.com/docs/capabilities/authorization) to an online instance of Visual Studio Code hosted by code-server.

This guide shows you how to secure code-server with Pomerium. The result allows devteams to safely expose their code-server and write code using VSCode in the browser:
