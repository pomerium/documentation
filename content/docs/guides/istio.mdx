...
import GrafanaIni from '../../examples/kubernetes/istio/grafana.ini.yml.md';
...
In our [Mutual Authentication section on Sidecars](/docs/concepts/mutual-auth#mutual-authentication-with-a-sidecar), we detail how a single service can offload authN and authz to a sidecar service. In a service mesh, each service in an internal network is deployed with a sidecar, and the controller configures them to provide mutual authentication with each other:
...
:::tip This is a simplified model that doesn't describe the additional traffic for authorization and authentication.

See the [Legend](/docs/concepts/mutual-auth#legend) on our Mutual Authentication page for details on our graphing style. :::
...
1. When [defining a test service](/docs/k8s/quickstart#test-service), you should now see two containers for the service pod:
...
1. Create a `grafana-values.yaml` file and add the annotations for the Pomerium Ingress Controller:
   ...
   :::tip Persistence is required to retain user data. Review the [Grafana Helm chart configuration](https://github.com/grafana/helm-charts/tree/main/charts/grafana#configuration) options to set the values for your environment. :::
...
[pomerium ingress controller]: /docs/deploying/k8s/ingress.md
[zero trust]: /docs/concepts/zero-trust.md#zero-trust
[install pomerium using helm]: /docs/guides/helm
