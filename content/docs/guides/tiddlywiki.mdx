...
This guide ships a sealed end-to-end test (`content/examples/guides/tiddlywiki/validate/`) that runs exactly these checks against an in-network identity provider on every change.
...
- **Redirect loop or certificate errors.** Make sure DNS for `tiddlywiki.yourdomain.com` points at Pomerium and that `autocert` could reach Let's Encrypt on ports 80/443.

## Security considerations

- TiddlyWiki blindly trusts whatever arrives in `authenticated-user-header`. If a client can reach TiddlyWiki directly, it can set that header and impersonate anyone. **Never publish the TiddlyWiki port.** Keep `tiddlywiki:8080` off published ports and on the internal Docker network so only Pomerium can reach it; Pomerium sets the header from the verified session.
- The email is forwarded in an **unsigned** header, which is safe only because the upstream is unreachable except through Pomerium. If you need the upstream to cryptographically verify the identity itself, use a [signed JWT](/docs/capabilities/getting-users-identity) instead; TiddlyWiki's header auth does not verify signatures.
