...
In this guide, you'll configure Pomerium to forward a user's email address in an unsigned header to the upstream application, TiddlyWiki. You'll configure TiddlyWiki to expect a header in a request using the `authenticated-user-header` parameter. (See the [ListenCommand](https://tiddlywiki.com/static/ListenCommand.html) documentation for more information.)

With this configuration, if an authorized user authenticates against Pomerium's hosted identity provider successfully, they won't have to enter their credentials again to access TiddlyWiki.
