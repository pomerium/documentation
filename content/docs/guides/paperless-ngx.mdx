...
The Compose file runs Pomerium Core alongside Paperless-ngx, PostgreSQL, and Redis. For Zero, drop the Core `pomerium` service, keep `paperless`, `db`, and `redis` on `paperless-internal`, and attach the Quickstart's `pomerium` service to `paperless-internal` so it can resolve `paperless` by name.
