...
sidebar_label: Configure Metrics
...
This guide shows you how to configure both the external and embedded Prometheus options.
...
1. In your Pomerium Enterprise configuration file, define the `metrics_addr` key to a network interface or port. For example:
...
1. In your Pomerium Enterprise configuration file, define the [`prometheus_url`](/docs/enterprise/configure#prometheus-url) key to point to your Prometheus instance:
...
Select **External Data** > **Metrics**. You should see monitoring data collected from the external data source record:
