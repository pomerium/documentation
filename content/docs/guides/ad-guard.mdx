...
The Compose file runs Pomerium Core and AdGuard together. Only port 53 (DNS) is published; the web UI on port 3000 stays on the internal network so it's reachable only through Pomerium. For Zero, drop the `pomerium` service and use the `compose.yaml` from the Quickstart with your `POMERIUM_ZERO_TOKEN`, keeping the `adguard` service below.
...
This guide ships a sealed end-to-end test (`content/examples/guides/ad-guard/validate/`) that runs these checks against an in-network identity provider on every change. The test seeds AdGuard past its install wizard, signs a user in through Pomerium, and asserts AdGuard's control API answers `200` from the injected credential. It also hits AdGuard directly (bypassing Pomerium) and asserts a `401`, proving the credential is what gates access.
...
- **AdGuard returns 401 / "Forbidden" through Pomerium.** The credential doesn't match. Re-run the install wizard or reset the password, then regenerate the base64 string from the new username and password.
...
- **Redirect loop or certificate errors.** Make sure DNS for `adguard.yourdomain.com` points at Pomerium and that `autocert` could reach Let's Encrypt on ports 80/443.
...
- **Don't expose AdGuard's web UI directly.** Only Pomerium should reach `adguard:3000`. Keep it off published ports and on the internal Docker network, as the Compose file does. If AdGuard is reachable on its own port, an attacker who knows (or brute-forces) the basic-auth password bypasses Pomerium entirely.
