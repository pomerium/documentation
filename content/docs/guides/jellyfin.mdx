...
lang: en-US
keywords: [pomerium, jellyfin, media server, sso, oidc, identity aware proxy, self-hosted]
...
3. Enable **Preserve Host Header** so Jellyfin's absolute URLs (web client, casting) match the **From** URL rather than the container name.
...
- **Redirects or absolute links point at the container name or the wrong host.** Jellyfin's `JELLYFIN_PublishedServerUrl` doesn't match the route's **From** URL, or `preserve_host_header` isn't set. Make both agree on `jellyfin.yourdomain.com`.
