...
The Compose file pins Forgejo to the `forgejo:15` major tag; bump it when you upgrade, and re-test sign-in afterward. To tear the stack down and delete the Forgejo data volume (all repositories and state), run `docker compose down -v`.
...
3. **SSO and auto-provisioning work.** Forgejo signs you in and creates your account from the headers, with no Forgejo password. The account carries your real email; its username comes from the configured username claim, and its full name is populated only if your identity provider supplies a `name` claim.
...
This guide ships a sealed end-to-end test (`content/examples/guides/forgejo/validate/`) that runs exactly these checks against an in-network identity provider on every change, including a test that a forged identity header is overwritten by Pomerium.
...
Reverse-proxy authentication means Forgejo believes the identity headers it receives. Forgejo does not verify Pomerium's signed `X-Pomerium-Jwt-Assertion` for login, so this integration depends entirely on **network isolation**: Forgejo must be reachable **only** through Pomerium. Pomerium itself always overwrites the inbound identity headers with the real authenticated user before forwarding, so a client that forges `X-Pomerium-Sub` or `X-Pomerium-Claim-Email` _through_ Pomerium is still signed in as its own identity, never the forged one. The risk is a client that bypasses Pomerium and talks to Forgejo directly: it can set those headers itself and impersonate anyone (the first such account would even become admin).
