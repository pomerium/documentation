...
The authenticate callback path is the path/url from the authenticate service
that will receive the response from your identity provider. This configuration
property is no longer available. `/oauth2/callback` will be used in all cases.

#### Stricter Internal Health Checking

Review our new documentation on [Health Checks](/docs/internals/health-checks)
to ensure the health checks are used effectively in your environment.

#### Token Refreshing Considers ID Token Expiration

Pomerium automatically refreshes Access Tokens with the identity provider while Pomerium sessions are still valid.
Now, Pomerium also ensures ID tokens it provides upstream are not expired by considering the ID Token expiration time.
ID tokens usually expire more quickly than Access Tokens, so this results in Access Tokens typically being refreshed much earlier than required.
