...
The authenticate callback path is the path/url from the authenticate service that will receive the response from your identity provider. This setting is no longer available. `/oauth2/callback` will be used in all cases.

#### Stricter Internal Health Checking

Review our new documentation on [Health Checks](/docs/internals/health-checks) to ensure the health checks are used effectively in your environment.

#### OIDC Session Refresh Considers ID Token Expiration

Pomerium automatically refreshes the OIDC session with the identity provider while a Pomerium session is still valid. Now, Pomerium will also refresh the OIDC session shortly before the expiration time of the ID token. If the identity provider supports refreshing the ID token, this allows Pomerium to maintain a valid ID token for the lifetime of the Pomerium session.

To revert this behavior, set the [Runtime Flag](/docs/reference/runtime-flags) `refresh_session_at_id_token_expiration` to `false`.
...
- A "Report Issue" feedback widget in the Enterprise Console, loaded from a third-party script. Disable with [`--disable-feedback-widget`](/docs/deploy/enterprise/configure).

##### Changed

- `--disable-validation` is now deprecated. Use [`--validation-mode=none`](/docs/deploy/enterprise/configure) to preserve existing behavior, or [`--validation-mode=static`](/docs/deploy/enterprise/configure). We'll remove `--disable-validation` in a future release.
...
- The `--disable-validation` option has been expanded to include additional validation modes, represented by [`--validation-mode`](/docs/deploy/enterprise/configure).
