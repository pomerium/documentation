...
Previously, the `certificates` key supported base64-encoded certificates as a value. We've removed support for base64-encoded certificates for this setting and now require that you only use the relative file location.

See [Certificates](/docs/reference/certificates#certificates) for more information.

#### Remove Redis Storage Backend

Support for Redis as a storage backend for Pomerium has been completely removed. We recommend a dedicated Postgres instance for Pomerium to provide the strongest security and performance guarantees.
...
#### Smaller configuration chunks

Previously, on each configuration change, Console would send the entire configuration to Core in a number of large chunks. Console would avoid sending any unchanged chunks, but due to the order of configuration objects within these chunks, a change affecting one chunk could “spill” over into all subsequent chunks, and so on average each update would still send a significant portion of the entire configuration.

Now, Console will send each settings, route, or certificate update to Core as a single record.

#### Databroker state sync

Previously, on every sync tick we would request all configuration chunks one by one in order to detect what need be changed (and what has to be deleted). In v24 we have a single databroker syncer.
