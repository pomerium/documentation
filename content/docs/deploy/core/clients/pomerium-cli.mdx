...
`pomerium-cli` (optional, Pomerium is clientless for HTTP based protocols) is a command-line client for working with Pomerium. Functions include acting as an authentication helper for tools like [kubectl](/docs/kubernetes/k8s/configure.md) or TCP [based applications](/docs/capabilities/tcp/).
