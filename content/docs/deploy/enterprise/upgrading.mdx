...
The Enterprise Console now includes a “Report issue” feedback widget, which is loaded from a third-party service. If you don't want to include this third-party script on your Enterprise Console installation, you can disable the feedback widget using the command-line option [`--disable-feedback-widget`](/docs/deploy/enterprise/configure#disable-feedback-widget), or by setting the environment variable [`DISABLE_FEEDBACK_WIDGET=true`](/docs/deploy/enterprise/configure#disable-feedback-widget).

#### Changed

The [`--disable-validation`](https://main.docs.pomerium.com/docs/deploy/enterprise/configure#disable-validation) configuration option is newly deprecated. If you are currently using it, please update your configuration to either use:

- [`--validation-mode=none`](/docs/deploy/enterprise/configure#validation_mode) to preserve the existing behavior, or
- [`--validation-mode=static`](/docs/deploy/enterprise/configure#validation_mode), to try a new lighter-weight validation mode.
...
- [Hosted Authenticate Service](/docs/capabilities/authentication.mdxx) will now be used by default to handle single-sign-on. Pomerium hosts this service as a convenience to its users; no identity provider configuration or authenticate service url needs to be specified if the hosted authenticate service is used. [Self-hosted authenticate service](/docs/capabilities/authentication.mdx) is still available for users who want to configure their own identity provider and authenticate service URL.
