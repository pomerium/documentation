...
[Set Request Headers](/docs/reference/routes/headers#set-request-headers) has three new token substitution values that you can forward to an upstream app or service:

- **Client certificate fingerprint**, which forwards the client certificate's fingerprint (downstream mTLS must be enabled)
- **ID token**, which forwards the OIDC ID token from the identity provider
- **Access token**, which forwards the OAuth access token from the identity provider

![Set client certificate fingerprint with the new set request headers option](./img/upgrading/v23/set-request-headers-fingerprint.png)

**Logging Configurations**<br/>

[Access Log Fields](/docs/reference/access-log-fields) and [Authorize Log Fields](/docs/reference/authorize-log-fields) settings allow you to easily review HTTP request logs from the Proxy and Authorize services.

![Configure logging fields in the Console](./img/upgrading/v23/logging-configs.gif)

**Cookies SameSite**<br/>

[Cookies SameSite](/docs/reference/cookies#cookie-samesite) is now configurable in the Enterprise Console.

![Configure Cookies SameSite in the Console](./img/upgrading/v23/cookie-samesite.gif)
