...
- [Set Request Headers](/docs/reference/routes/headers#set-request-headers) has three new token substitution values: **client certificate fingerprint**, which forwards the client certificate's fingerprint to the upstream app or service; **ID token**, which forwards the OIDC ID token from the identity provider; and **access token**, which forwards the OAuth access token from the identity provider.
- [Access Log Fields](/docs/reference/access-log-fields) and [Authorize Log Fields](/docs/reference/authorize-log-fields) settings make it easier for you to review HTTP request logs from the Proxy and Authorize services.
