...
In v0.26, we made some changes to the Enterprise Console's PPL builder. These updates include:

- **Client certificate fingerprint criterion**: Enables you to specify a client certificate fingerprint value in the PPL builder
- **Numerical comparison operators**: When building policies that use the `Record` external data criterion, numerical comparison operators allow you to specify the user's minimum device posture score, which is derived from the user's client certificate fingerprint.
- **Client Trusted Certificate PPL override setting**: This new override setting instructs Pomerium to require a valid client TLS certificate before proxying a request.

### Changed

This release includes various changes to OPA, PPL, the Enterprise API, and the Enterprise Console's UI.
