...
After installation, see how to configure Pomerium using a minimal example, plus notes on starting and running the service.

## Prerequisites

- A configured [Identity Provider](/docs/identity-providers)
- Properly [TLS certificates](/docs/guides/certificates) if you plan to serve on `443` or otherwise require HTTPS

:::note

In a local development or proof-of-concept scenario, you can use [mkcert](https://mkcert.dev/) or other self-signed certificate solutions. For production, consider Let’s Encrypt or a private CA in your organization.
...
For local testing, specify the `certificate_file` and `certificate_key_file` if using mkcert or other local certs. In production, you may rely on Let’s Encrypt or external cert manager. See [TLS certificates](/docs/guides/certificates) for details.
