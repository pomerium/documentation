...
1. Go back to the **App registrations** page and select your app. Select the **Endpoints** tab.
   ![From the app registration page, select the Endpoints tab](./img/entra/entra-endpoints-tab.png)
...
As of this writing, Pomerium only supports **`azure`** as the valid **`idp_provider`** name to reference Microsoft Entra ID as an identity provider (not **`entra`**).

This means you **do not** need to update **`azure`** in your configuration file. If you update the **`idp_provider`** key to **`entra`**, it will break your configuration.
...
1. Go to **App registrations** and select **Token configuration**, then **+ Add groups claim**.
  ![Select Token Configuration under the Manage section in the sidebar](./img/entra/entra-token-configuration.png)
1. In the **Edit groups claim** panel, select the appropriate group types. Under **Customize token properties by type**, select **Group ID** for **ID**, **Access**, and **SAML**.
![Edit the tokens groups claim in the Edit groups claim panel](./img/entra/entra-edit-groups-claim.png)

### Apply custom claims to a policy

You can use the [Claim PPL criterion](/docs/capabilities/ppl#criteria) to factor in the `groups` value in your authorization policy.
