...
To use `allowed_groups` in a policy, an `idp_service_account` needs to be set in the Pomerium configuration. The service account for OneLogin uses a _different_ client ID and client secret from the one configured above.
...
[identity provider url]: /docs/reference/identity-provider-url
[environmental variables]: https://en.wikipedia.org/wiki/Environment_variable
[oauth2]: https://oauth.net/2/
[openid connect]: https://en.wikipedia.org/wiki/OpenID_Connect
[onelogin]: https://www.onelogin.com/
[installed pomerium]: /docs/install/
