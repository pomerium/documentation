...
This guide requires you to self-host Keycloak. To minimize set up time, you will deploy Pomerium, Keycloak, and the Pomerium Verify service in Docker containers.
...
1. In the **Realm name** field, enter a name
1. Select **Create**
...
1. Go to the admin console (`0.0.0.0:8080/admin`)
1. Select **Users**
1. Select **Add users**
...
1. In the **Client ID** field, enter a name (for example, `myclient`)
...
1. In the **Valid redirect URIs** field, enter `https://authenticate.localhost.pomerium.io/oauth2/callback`
1. Select **Save**

![Add Keycloak access settings](./img/oidc/keycloak-access-settings.png)
...
- The value of `idp_client_id` should be `account` — not the client ID you assigned the client when you created it
...
Because this example secures Pomerium's Verify app, you should add a signing key so Pomerium can sign the Pomerium JWT. This will allow you to review a user's JWT claims.

Generate a **signing key** with the following command:
...
Add the value to `signing_key`.

:::note

See [**Identity Verification**](/docs/capabilities/getting-users-identity) for more information on how Pomerium handles identity verification with JWTs.

See [**Signing Keys**](/docs/reference/signing-key) for more information on generating and using signing keys.
...
Pomerium should redirect you to the Keycloak realm you created. Sign in with your user's credentials.
...
## Next steps: Add groups and roles (Enterprise)
