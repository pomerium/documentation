...
title: Ping
lang: en-US
keywords: [ping, oidc, identity provider, idp]pagination_prev: null
...
To use `allowed_groups` in a policy, an `idp_service_account` needs to be set in the Pomerium configuration. The service account for Ping uses a _different_ application, and client ID and client secret from the one configured above.
