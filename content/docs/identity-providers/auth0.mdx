...
title: Auth0
lang: en-US
keywords: [auth0, pomerium, identity provider, idp]pagination_prev: null
...
This page documents configuring an [Auth0] Web Application and Machine to Machine Application for Pomerium to read user data. It assumes you have already [installed Pomerium](/docs/install/).
...
To authorize users based on their group membership (roles in Auth0), a claim can be added to the identity and access tokens with an [login action](https://auth0.com/docs/customize/actions).
...
1. Create an action named `add groups` with the following code:
   ...
   exports.onExecutePostLogin = async (event, api) => {
     if (event.authorization) {
       api.idToken.setCustomClaim("pomerium.io/groups", event.authorization.roles);
       api.accessToken.setCustomClaim("pomerium.io/groups", event.authorization.roles);
     }
   };
   ```
