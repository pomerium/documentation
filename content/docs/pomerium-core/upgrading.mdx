...
`pomerium-cli` has been factored out of the core repository and now resides at <https://github.com/pomerium/cli>. If you currently install the CLI tool from [Packages](/docs/pomerium-core/core#packages-2) or [Homebrew](/docs/pomerium-core/core#homebrew), no changes should be required to your process. However, users of docker images or direct github release downloads will need to update their references.

Please see the [updated install instructions](/docs/pomerium-core/pomerium-cli) for additional details.
