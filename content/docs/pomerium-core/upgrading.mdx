...
- `idp_refresh_directory_timeout`: use [Polling Min Delay](/docs/enterprise/configure#polling-minmax-delay).
- `idp_refresh_directory_interval`: replaced by [Polling Max Delay](/docs/enterprise/configure#polling-minmax-delay).
...
`pomerium-cli` has been factored out of the core repository and now resides at <https://github.com/pomerium/cli>. If you currently install the CLI tool from [Packages](/docs/pomerium-core/install/core#packages-2) or [Homebrew](/docs/pomerium-core/install/core#homebrew), no changes should be required to your process. However, users of docker images or direct github release downloads will need to update their references.

Please see the [updated install instructions](/docs/pomerium-core/install/pomerium-cli) for additional details.
