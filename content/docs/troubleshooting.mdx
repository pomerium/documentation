...
Pomerium's independent services communicate securely using a [shared secret](/docs/reference/shared-secret). When Pomerium services exchange a mismatched secret, or these services exchange a secret that is not the same secret set in the Databroker service, Pomerium will fail.
