...
Upstream connection errors indicate that something is wrong with the upstream server, not Pomerium. Please refer to the list of upstream connection errors below to learn more about the issue, and how you can resolve it.

| Upstream error | Description |
| :-- | :-- |
| no_healthy_upstream | Some description |

### No healthy upstream

"No healthy upstream" means that there is an issue with the upstream server that's causing is to the request was rejected by the router filter because there was no healthy upstream found.
