...
Pomerium's independent services communicate securely using a [shared secret](/docs/reference/shared-secret). When Pomerium services contain a mismatched secret, or these services contain a secret that is not the same secret set in the Databroker service, Pomerium will fail.
