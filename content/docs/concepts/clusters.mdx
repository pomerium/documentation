...
- There is less configuration to maintain, which makes Pomerium easier to manage
...
Replicas within a cluster share the same storage backend to synchronize records and session state. In this way, you can deploy multiple replicas to meet the needs of your infrastructure and organization.
...
TODO: Mention that deploying a replica is not done in the UI.
