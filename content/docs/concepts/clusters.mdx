...
1. Copy the token value and store it somewhere safe
  ![Rotating a cluster identity in the Zero Console](./img/clusters/pz-rotate-cluster-identity.gif)
...
In the Zero Console, the cluster's IP address is referred to as the **Detected IP Address**. Depending on your environment, the detected IP address may not be able to reach you, the end-user. This could be the case if, for example, you're running a cluster:
