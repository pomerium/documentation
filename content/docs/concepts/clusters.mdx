...
When you deploy Pomerium Zero, you get your own **Cluster**. A cluster consists of one ore more replicas of [Pomerium Core](/docs/deploy/core), the primary server component that secures your services. Clusters are deployed locally and managed through the **Zero Console**, a remote control plane hosted in the Pomerium Zero cloud.

## Why clusters?

A cluster model reduces the complexity of managing Pomerium: All configuration settings, including service accounts, routes, authorization policies, and certificates, are maintained in the Pomerium Zero cloud.

Each replica within a cluster runs Pomerium Core in an all-in-one mode, meaning the services that comprise Core are deployed together and share session state. The diagram below provides a high-level overview of how these services communicate:

   ![A diagram depicting the Pomerium Core service identities](./img/clusters/core-architecture.png)

Similary, replicas communicate with each other to synchronize state across a cluster. This interconnectivity between replicas enables you to horizontally scale your cluster to adapt to the purposes of your use case, be it performance, governance, security, or management related.
...
- You don't have to manage cluster configuration files or environment variables locally, which reduces the complexity of managing Pomerium Zero
- The Zero Console maintains your cluster's last-known configuration, so replicas can continue running in the event of a service outage
...
To avoid failover, we recommend that you deploy a PostgreSQL database to persist your cluster's configuration state. Since all replicas share the same storage connection, the database will persist cluster state if a replica restarts due to a service outage.
...
1. Enter the connection string in the [**Databroker Storage Connection String**](/docs/reference/databroker#databroker-storage-connection-string) field
  ...
  ![Entering the databroker storage connection string in the Zero Console](./img/clusters/databroker-connection.png)

### Replicas

You can't deploy replicas in the Zero Console itself. To deploy a replica, run the Pomerium Zero installation script using the same [cluster token](#cluster-token) you used to deploy your cluster. This will generate another replica, which you can verify on the **Status** page.
