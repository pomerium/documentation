...
Cluster configuration is fully managed in Pomerium Zero. Configuration changes made in the Zero are fetched and applied locally by your cluster.

This model ensures that:

- Your cluster remains synchronized with the Zero Console as changes are applied
...
Pomerium cannot and does not monitor your organization's traffic or collect private user data. Pomerium does, however,  needs to store configuration details required to maintain the hosted control plane. Pomerium only stores deployment and configuration details needed to control access to your services, such as routes, policies, and TLS certificates and encrypts sensitive entities when appropriate.
