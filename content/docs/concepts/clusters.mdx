...
In the patch command below, replace the value of `pomerium_zero_token` with your new cluster token:

  ```yaml
  kubectl -n pomerium patch secret pomerium-zero-token -p '{"stringData":{"pomerium_zero_token":"<cluster_token>"}}'
  ```

:::note

The cluster token generated by Pomerium Zero is already [base64URL](https://base64.guru/standards/base64url) encoded.
