...
When you deploy Pomerium Zero, you get your own **Cluster**. A cluster consists of one or more replicas of [Pomerium Core](/docs/deploy/core), the primary server component that secures your services. Clusters are deployed locally and managed through Pomerium Zero's hosted control plane.

## Why clusters?

A cluster model reduces the complexity of managing Pomerium: All of a cluster's configuration, including settings, routes, policies, and certificates, are managed in Pomerium Zero.
...
Cluster configuration is fully managed in Pomerium Zero. Configuration changes made in Pomerium Zero are fetched and applied locally by your cluster.
...
Pomerium cannot and does not monitor your organization's traffic or collect private user data. However, Pomerium does need to store configuration details required to maintain the hosted control plane.
...
When you deploy a cluster in Pomerium Zero, a **Cluster Token** is generated for you during onboarding. The cluster token environment variable instructs Pomerium to run in a special "Zero-managed" mode, wherein your cluster connects to the Pomerium Zero cloud.
...
Whenever you deploy a cluster, it must fetch its configuration from Pomerium Zero. During this process, Pomerium Zero attempts to detect the cluster's outbound IP address to connect to it.

In Pomerium Zero, the cluster's IP address is referred to as the **Detected IP Address**. Depending on your environment, the detected IP address may not be reachable by you, the end-user. This could be the case if, for example, you're running a cluster:
