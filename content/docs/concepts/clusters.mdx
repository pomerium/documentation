...
Each replica within a cluster runs Pomerium Core in an all-in-one mode, meaning the services that comprise Core are deployed together and share session state. Similary, replicas communicate with each other to synchronize state across a cluster.
