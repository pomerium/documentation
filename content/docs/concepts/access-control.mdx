---
sidebar_label: Access (AuthN/AuthZ)
lang: en-US
keywords:
  [
    access,
...
**Authentication** (AuthN) is the process of determining if you are who you say you are.

**Authorization** (AuthZ) is the process of determining if you are allowed to do the thing you are trying to do.
...
Authorization policy can be expressed in a high-level, [declarative language](/docs/capabilities/ppl) or [as code](/docs/capabilities/ppl) that can be used to enforce ABAC, RBAC, or any other governance policy controls. Pomerium can make holistic policy and authorization decisions using external data and request context factors such as user groups, roles, time, day, location and vulnerability status.
...
[reference: policies]: /docs/capabilities/policies
[reference: namespace]: /docs/concepts/namespacing
[reference: routes]: /docs/capabilities/routing
...
- Manually add the user's unique ID. Identify the ID from a user's Session Details page, or the [Sessions](/docs/capabilities/reports#sessions) page in Pomerium Enterprise.
