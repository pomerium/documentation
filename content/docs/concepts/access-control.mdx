...
- Manually add the user's unique ID. Identify the ID from a user's **Session Details** page, or the [Sessions](/docs/capabilities/reports#sessions) page in [Pomerium Enterprise](/docs/deploy/enterprise). A user can see their session ID by navigating to the special `/.pomerium` URL endpoint from any Pomerium-managed route. The unique ID is listed as **Sub** under User Details:
