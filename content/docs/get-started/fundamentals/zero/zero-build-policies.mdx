...
**Example 3**: Allow access based on the User ID criterion

Email and domain policies match users by email address, but you can also write a policy that allows access if a user has a specific `user` ID.

In the Zero Console, select **User ID** from the **Criteria** dropdown, keep the **Operator** set to **Is**, and enter the user's ID in the **Value** field.

    ![Creating a policy in the Zero Console that grants access based on the User ID criterion](./img/zero-policies/pz-courses-ppl-example-3.png)
