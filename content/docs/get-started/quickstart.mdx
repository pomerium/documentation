...
- An [identity provider](/docs/integrations/user-identity/identity-providers) configured for authentication
...
Pomerium requires two secrets:

- [**Shared Secret**](/docs/reference/shared-secret): Used to authenticate requests between Pomerium services
- [**Cookie Secret**](/docs/reference/cookies#cookie-secret): Used to encrypt and sign session cookies
...
Save these values—you'll add them to your config file next.

## Set up identity provider

Configure your [identity provider](/docs/integrations/user-identity/identity-providers). When setting up OAuth credentials, use this as your redirect URI:

```
https://authenticate.pomerium.yourdomain.com/oauth2/callback
```
...
# Authenticate service URL (must match your DNS)
authenticate_service_url: https://authenticate.pomerium.yourdomain.com

# Secrets (replace with your generated values)
shared_secret: REPLACE_WITH_GENERATED_SECRET
cookie_secret: REPLACE_WITH_GENERATED_SECRET

# Identity provider settings
idp_provider: REPLACE_WITH_IDP_PROVIDER
idp_client_id: REPLACE_WITH_IDP_CLIENT_ID
idp_client_secret: REPLACE_WITH_IDP_CLIENT_SECRET
...
- `REPLACE_WITH_GENERATED_SECRET` values with your generated secrets
- `REPLACE_WITH_IDP_*` values with your [identity provider](/docs/integrations/user-identity/identity-providers) settings
...
services:
  pomerium:
    ...
    volumes:
      ...
      - pomerium-cache:/var/cache/pomerium
    ports:
      - '443:443'
      - '80:80'
...
The `pomerium-cache` volume stores Let's Encrypt certificates so they persist across container restarts.
