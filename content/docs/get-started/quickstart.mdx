...
The `pomerium-cache` volume will store Let's Encrypt certificates so they persist across container restarts.
...
If you stop and restart the Pomerium container at this point, the next time you access the Pomerium route you may be prompted to sign in again. To avoid this, you'll need to configure Pomerium with a fixed [shared secret](/docs/reference/shared-secret) and a [persistent storage backend](/docs/reference/databroker).

To generate a new shared secret you can run this command:

```
head -c32 /dev/urandom | base64
```

Then add these lines to your config.yaml file:

```yaml title="config.yaml" {1-4}
...
- [Set up a Custom Identity Provider](/docs/get-started/fundamentals/core/self-hosted-pomerium)
