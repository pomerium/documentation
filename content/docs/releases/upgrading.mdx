...
For Enterprise, use PPL Builder ![policy_groups_enterprise](img/policy_groups_enterprise.png)
...
`http_path` and `http_method` are now supported for matching HTTP requests in policies. See [Pomerium Policy Language](/docs/concepts/ppl#criteria) for more details.
...
See [Load Balancing](/docs/concepts/load-balancing) for more information on using this feature set.
...
Previous programmatic authentication endpoints (`/api/v1/token`) has been removed and has been replaced by a per-route, oauth2 based auth flow. Please see updated [programmatic documentation](/docs/concepts/programmatic-access) how to use the new programmatic access api.
