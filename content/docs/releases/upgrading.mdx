...
The [Hosted Authenticate Service](/docs/capabilities/hosted-authenticate-service.md) provides a hosted authenticate service URL and hosted identity provider. The [Self-Hosted Authenticate Service](/docs/capabilities/self-hosted-authenticate-service), which requires a self-hosted authenticate service URL and identity provider, is still supported for users that prefer to host these services themselves.
