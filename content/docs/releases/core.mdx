...
We recommend following the steps in the Kubernetes [Installation](/docs/deploying/k8s/install) guide to deploy Pomerium with Kubernetes, or see the Kubernetes [Quickstart](/docs/deploying/k8s/quickstart) for a proof of concept of how to configure and deploy Pomerium with Kubernetes.
