...
The [Hosted Authenticate Service](/docs/capabilities/hosted-authenticate-service) provides a hosted authenticate service URL and identity provider that runs by default. You can still use the [Self-Hosted Authenticate Service](/docs/capabilities/self-hosted-authenticate-service) if you want to host these services yourself.

##### Wildcard From Routes

[Wildcard From Routes](/docs/reference/routes/from#wildcard-from-routes) support allows you to define multiple near-identical external routes that point to the same internal route, reducing duplication.
