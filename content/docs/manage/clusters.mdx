...
Configuration changes applied in the hosted control plane are communicated over a streaming connection to your local cluster. Each cluster is connected to its own storage backend, which synchronizes state across replicas.
    ...
    ![A diagram depicting how clusters communicate with the Zero Console](./clusters/clusters-architecture.png)
...
![Rotating the cluster token in the Zero Console](./clusters/update-cluster-token.png)
...
![Manually setting the cluster IP address in the Zero Console](./clusters/cluster-ip-address-settings.png)
...
![Viewing the new cluster name in the Zero Console](./clusters/cluster-name-table.png)
...
![Entering the databroker storage connection string in the Zero Console](./clusters/zero-databroker-connection-string.png)
...
![Reviewing Replica status in the Zero Console](./clusters/replica-status.png)
