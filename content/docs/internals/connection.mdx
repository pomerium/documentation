...
| `ssh` | TCP/SSH | Native SSH proxy | 22 (default) | [`ssh_address`](../capabilities/native-ssh-access#configuration-reference) |
...
Native SSH proxy listener created when SSH access capability is enabled and [`ssh_address`](../capabilities/native-ssh-access#configuration-reference) is set. Supports policy-based access control over SSH.
