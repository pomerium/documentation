...
- The cluster token may have been rotated without updating the replicas

**Steps to resolve**: Verify that at least one replica is running and can connect to the Pomerium Zero cloud. Check your replica logs for connection errors and ensure the cluster token is current.
