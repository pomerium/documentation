...
### Cluster has no active replicas \{#cluster_active_replicas}

This cluster status alert checks whether a cluster has at least one active replica connected to Pomerium Zero. It runs periodically to verify cluster health.

**What causes this alert**: None of the cluster's replicas are currently connected to the Pomerium Zero cloud. This could mean:

- This is a new cluster, and a Pomerium replica has not been started with the cluster token yet
- All replicas have been stopped or are unreachable
- Network connectivity issues are preventing replicas from connecting
- The [cluster token](#rotate-cluster-token) may have been rotated without updating the replicas
- A replica was started with a token from a different cluster or organization, so this cluster still reports no connected replicas

**Steps to resolve**:

1. If this is a new cluster, deploy at least one replica by following the [Install Pomerium Zero](/docs/deploy/cloud/install) guide for your environment.
1. If replicas are already deployed, make sure at least one process or pod is running and has the current `POMERIUM_ZERO_TOKEN` value. If you rotated the cluster token, update the replica configuration and restart it.
1. Check the replica logs for authentication or connection errors. The replica must be able to reach the Pomerium Zero cloud over HTTPS.
1. After a replica starts, wait for the periodic status check to update, then refresh the Zero Console [Status](https://console.pomerium.app/app/reports/status) page. The **Check Again** button can re-run health checks on connected replicas, but this active-replica status updates on its periodic schedule.

### Configuration build failed {#config_build}
...
This cluster status alert verifies that all configured routes can be reached via DNS and that the Pomerium JWKS endpoint is accessible for each route host. It runs every time the cluster configuration is updated.

**What causes this alert**: One or more route hostnames cannot be resolved or reached. This could mean:

- DNS records for the route hostnames are missing or incorrect
...
This cluster status alert tracks whether Zero resource bundles (configuration packages) were successfully applied to the cluster. It runs when configuration bundles are received from the Pomerium Zero cloud.

**What causes this alert**: A configuration resource bundle could not be applied to the cluster. This could be due to:

- Version incompatibility between the resource bundle and the running Pomerium version
- Corruption or incomplete transmission of the resource bundle
- Internal errors during bundle application

**Steps to resolve**: If this error persists, try restarting your cluster replicas. If the issue continues, please [contact support](https://discuss.pomerium.com/).

### Telemetry collection failed {#zero.telemetry.collect-and-send}

This cluster status alert monitors whether telemetry data was successfully collected and sent to the Pomerium Zero cloud. It runs when the cloud requests telemetry data from the cluster.

**What causes this alert**: The cluster was unable to collect or transmit telemetry metrics to Pomerium Zero. This could be due to:

- Connectivity issues between the cluster and Pomerium Zero
- Internal errors during metrics collection
- Resource constraints preventing telemetry processing

**Steps to resolve**: This is typically a transient issue that resolves automatically. If the error persists, check network connectivity and restart your cluster replicas. For ongoing issues, please [contact support](https://discuss.pomerium.com/).
