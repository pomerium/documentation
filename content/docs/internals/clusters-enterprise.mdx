...
Each Pomerium Core installation needs to be configured with its own [shared secret](docs/reference/shared-secret), [databroker service URL](/docs/reference/service-urls#databroker-service-url), and should use its own database. See [Pomerium Core](/docs/deploy/core) and [Pomerium Enterprise](/docs/deploy/enterprise/install) for more details.
...
**Name**, **Databroker Service URL**, and **Shared Secret** are required. **Override Certificate Name**, **Certificate Authority**, and **Certificate Authority File** are only needed for [secure gRPC connections](docs/reference/grpc-insecure) to the databroker.
