...
To configure a redirect in the Zero Console:
...
1. In the **Host Redirect** field, enter a URL you want to redirect requests to, and remove the scheme
   ...
   For example: ![Adding a redirect in the Zero Console](./img/advanced-routes/add-redirects-in-zero-console.png)
...
![Demonstrates how Pomerium redirects user to the redirect URL](./img/advanced-routes/pz-redirect.gif)

### Direct Response

You can configure Pomerium to send a small, static HTTP response to the downstream client for a managed route.

To configure a [**Direct Response**](/docs/reference/routes/direct-response) in the Zero Console:
